...
class BluetoothIO extends BluetoothGattCallback {
    ...
    public boolean writeCharacteristic1(UUID characteristicUUID, byte[] value, ActionCallback callback) {
        return writeCharacteristic1(Profile.UUID_SERVICE_MILI, characteristicUUID, value, callback);
    }

    public void writeCharacteristic(UUID serviceUUID, UUID characteristicUUID, byte[] value, ActionCallback callback) {
        try {
            if (null == gatt) {
                Log.e(TAG, "writeCharacteristic connect to miband first");
                throw new Exception("writeCharacteristic error [connect to miband first]");
            }
    ...
    public boolean writeCharacteristic1(UUID serviceUUID, UUID characteristicUUID, byte[] value, ActionCallback callback) {
        try {
            if (null == gatt) {
                Log.e(TAG, "writeCharacteristic connect to miband first");
                return false;
    ...
    public void setNotifyListener(UUID serviceUUID, UUID characteristicId, NotifyListener listener) {
        if (null == gatt) {
            Log.d(TAG, "[setNotifyListener] connect to miband first");
            return;
        }

        BluetoothGattCharacteristic chara = gatt.getService(serviceUUID).getCharacteristic(characteristicId);
        if (chara == null) {
            Log.d(TAG, "[setNotifyListener] characteristicId " + characteristicId.toString() + " not found in service " + serviceUUID.toString());
            return;
        }
        try {
            ...
            Log.d(TAG, "[setNotifyListener] error=" + e.toString());
        }
    }

    public boolean setNotifyListener1(UUID serviceUUID, UUID characteristicId, NotifyListener listener) {
        if (null == gatt) {
            Log.d(TAG, "[setNotifyListener] connect to miband first");
            return false;
        }
        ...
        try {
            ...
            Log.d(TAG, "[setNotifyListener1] error=" + e.toString());
            return false;
        }
    }
    ...
    public void onConnectionStateChange(BluetoothGatt gatt, int status, int newState) {
        super.onConnectionStateChange(gatt, status, newState);
            if (newState == BluetoothProfile.STATE_CONNECTED) {
                Log.d(TAG, "BluetoothIO gatt is connected mac="+gatt.getDevice().getAddress());
                try {
                    Thread.sleep(500);
                }catch (Exception e){
                    Log.d(TAG,"onConnectionStateChange sleep error="+e.toString());
                }
                gatt.discoverServices();
            } else if (newState == BluetoothProfile.STATE_DISCONNECTED) {
                Log.d(TAG, "BluetoothIO gatt is closed mac="+gatt.getDevice().getAddress());
                gatt.close();
    ...
    public void onServicesDiscovered(BluetoothGatt gatt, int status) {
        ...
        if (status == BluetoothGatt.GATT_SUCCESS) {
            this.gatt = gatt;
            this.onSuccess(null);
            Log.d(TAG, "onServicesDiscoveredt gatt");
        } else {
            gatt.close();

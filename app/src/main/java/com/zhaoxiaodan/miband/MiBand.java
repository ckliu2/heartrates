...
public class MiBand {
    ...
    public boolean setUserInfo1(UserInfo userInfo) {
        try {
            BluetoothDevice device = this.io.getDevice();
            byte[] data = userInfo.getBytes(device.getAddress());
            return this.io.writeCharacteristic1(Profile.UUID_CHAR_USER_INFO, data, null);
        } catch (Exception e) {
            Log.d(TAG, "setUserInfo1 error=" + e.toString());
            return false;
        }
    }
    ...
    public void setHeartRateScanListener(final HeartRateNotifyListener listener) {
        this.io.setNotifyListener(Profile.UUID_SERVICE_HEARTRATE, Profile.UUID_NOTIFICATION_HEARTRATE, new NotifyListener() {
            @Override
            public void onNotify(byte[] data) {
                Log.d(TAG, "setHeartRateScanListener------" + getDevice().getAddress() + "-----" + Arrays.toString(data));
    ...
    public boolean setHeartRateScanListener1(final HeartRateNotifyListener listener) {
        try {
            return this.io.setNotifyListener1(Profile.UUID_SERVICE_HEARTRATE, Profile.UUID_NOTIFICATION_HEARTRATE, new NotifyListener() {
                @Override
                public void onNotify(byte[] data) {
                    Log.d(TAG, "setHeartRateScanListener------" + getDevice().getAddress() + "-----" + Arrays.toString(data));
                    if (data.length == 2 && data[0] == 6) {
            ...
            Log.d(TAG, "setHeartRateScanListener1 error=" + e.toString());
            return false;
        }
    }
    ...
    public void disconnect() {
        MiBand.this.io.gatt.disconnect();
    }

    public void gattClose() {
        MiBand.this.io.gatt.close();
    }
